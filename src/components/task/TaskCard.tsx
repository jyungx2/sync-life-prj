...
export default function TaskCard({
  ...
  return (
    <Draggable draggableId={task.id} index={index}>
      {(provided, snapshot) => {
        const dimmed = isSearchActive && !isMatched;
        const glowing = isSearchActive && isMatched;
        ...
        return (
          <div
            ...
            className={`
              bg-white rounded-[0.8rem] border border-grey-30 p-[1.6rem] cursor-pointer
              transform-gpu will-change-transform
              transition-[opacity,filter,transform,box-shadow,border-color] duration-150 ease-out

              ${snapshot.isDragging ? "shadow-lg transition-none" : "shadow-sm hover:shadow-md"}

              ${dimmed ? "opacity-35 blur-[1px]" : ""}
              ${glowing ? "ring-2 ring-violet-300 border-violet-300 shadow-lg scale-[1.01]" : ""}
            `}
            onClick={onEdit}
          >
            {/* 헤더: 제목 + 삭제 버튼 */}
